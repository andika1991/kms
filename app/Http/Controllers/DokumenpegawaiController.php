...
class DokumenpegawaiController extends Controller
{
    ...
    public function create()
    {
        ...
        // Ambil kategori: yang umum (NULL) atau milik subbidang user
        $kategori = \App\Models\KategoriDokumen::with('subbidang')
            ->when($subbidangId, function ($q) use ($subbidangId) {
                $q->where(function ($qq) use ($subbidangId) {
                    $qq->whereNull('subbidang_id')
                    ->orWhere('subbidang_id', $subbidangId);
                });
            })
            ->orderBy('nama_kategoridokumen')
            ->get();

        // Fallback: jika tetap kosong, tampilkan semua kategori agar dropdown tidak blank
        if ($kategori->isEmpty()) {
            $kategori = \App\Models\KategoriDokumen::orderBy('nama_kategoridokumen')->get();
        }
