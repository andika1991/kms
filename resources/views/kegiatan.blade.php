...
<section class="relative pb-16">
    <div class="absolute top-0 left-0 right-0 h-96 w-full">
        <div class="h-full w-full bg-cover bg-center" style="background-image: url('{{ asset('assets/img/background_section_kegiatan.png') }}');"></div>
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
                ...
                <p class="text-white/90 text-sm mt-1 max-w-xl drop-shadow">
                    Berbagai aktivitas atau program yang dilaksanakan oleh Dinas Komunikasi, Informatika dan Statistik Provinsi Lampung.
                </p>
            </div>
            <form action="{{ route('kegiatan') }}" method="GET" class="relative mt-4 md:mt-0 w-full max-w-xs">
                <input name="q" type="text" placeholder="Cari Kegiatan..." class="w-full bg-transparent placeholder-white/80 text-white border-b-2 border-white/50 py-2 pr-10 pl-2 outline-none focus:border-white transition-colors duration-300 text-base" autocomplete="off" />
                <button type="submit" class="absolute right-2 top-1/2 -translate-y-1/2 text-white/80 hover:text-white" aria-label="Search">
        ...
        <div class="swiper-container portal-kegiatan-slider mb-10">
            <div class="swiper-wrapper">
                ...
                <div class="swiper-slide">
                    <a href="/kegiatan/detail/{{ $item->id }}" class="block rounded-xl overflow-hidden relative group h-80">
                        <img src="{{ asset('storage/' . ($item->fotokegiatan[0]->path_foto ?? 'default.jpg')) }}" alt="{{ $item->nama_kegiatan }}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300">
                        <div class="absolute inset-0 bg-gradient-to-t from-black/70 via-black/20 to-transparent"></div>
                        <div class="absolute bottom-0 left-0 p-5">
                            <h2 class="text-white text-xl font-bold leading-tight drop-shadow-md">{{ $item->nama_kegiatan }}</h2>
                        </div>
                    </a>
                </div>
                @empty
                <div class="swiper-slide">
                    <div class="h-80 flex items-center justify-center text-gray-500 bg-gray-100 rounded-xl">Tidak ada kegiatan untuk ditampilkan</div>
                </div>
                @endforelse
            </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-x-8 gap-y-10">
            ...
            <div class="lg:hidden relative">
                <div class="overflow-x-auto -mx-4 px-4 pb-2" id="scrollBidangKegiatanMobile">
                    <ul class="flex gap-4 w-max pr-6" id="listBidangKegiatanMobile">
                        ...
                        <li class="bidang-item-kegiatan flex-shrink-0 flex items-center gap-2 cursor-pointer group px-3 py-2 rounded-lg hover:bg-blue-50 transition-colors border border-gray-200 bg-white" data-id="{{ $bidang->id }}">
                            <i class="fas fa-folder-open text-blue-600 group-hover:text-blue-800 text-lg"></i>
                            <span class="text-sm font-medium text-gray-700 group-hover:text-blue-800">{{ $bidang->nama }}</span>
                        </li>
                        @endforeach
                    </ul>
                </div>
            ...
            <aside class="lg:col-span-1 h-fit hidden lg:block">
                ...
                <ul class="flex flex-col gap-4" id="listBidangKegiatan">
                    @foreach ($bidangs as $bidang)
                    <li class="bidang-item-kegiatan flex items-center gap-4 cursor-pointer group px-2 py-2 rounded-lg hover:bg-blue-50 transition-colors" data-id="{{ $bidang->id }}">
                        <span class="bg-blue-100 flex items-center justify-center rounded-lg w-9 h-9 shadow-sm transition-transform group-hover:scale-105 group-hover:bg-blue-500">
                            <i class="fas fa-folder-open text-blue-600 group-hover:text-white transition-colors text-lg"></i>
                        </span>
                        <span class="font-semibold text-gray-700 group-hover:text-blue-800 text-base tracking-tight">{{ $bidang->nama }}</span>
                    </li>
                    @endforeach
                </ul>
            </aside>
...
<p class="text-sm text-gray-500 mb-1">Waktu: ${kegiatan.created_at ? kegiatan.created_at.substring(0, 10) : '-'}</p>
