    ...
    <div x-data="{ showSaveModal: false }"
        ...
        <div
            ...
            <form id="editKategoriForm"
                action="{{ route('admin.kategoripengetahuan.update', $kategoripengetahuan->id) }}" method="POST"
                class="w-full space-y-4" @submit.prevent="showSaveModal = true">
                @csrf
                @method('PUT')

                {{-- Dropdown Pilih Bidang --}}
                <label for="bidang_id" class="block font-semibold text-gray-700 mb-1">Bidang</label>
                <select name="bidang_id" id="bidang_id" required
                    class="w-full rounded-xl border border-gray-300 px-4 py-3 focus:outline-none focus:ring-2 focus:ring-blue-500 shadow transition text-base">
                    <option value="">-- Pilih Bidang --</option>
                    @foreach ($bidangs as $bidang)
                        <option value="{{ $bidang->id }}"
                            {{ old('bidang_id', $kategoripengetahuan->bidang_id) == $bidang->id ? 'selected' : '' }}>
                            {{ $bidang->nama_bidang }}
                        </option>
                    @endforeach
                </select>
