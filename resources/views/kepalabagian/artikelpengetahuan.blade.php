        ...
        <div class="p-6 md:p-8 grid grid-cols-1 xl:grid-cols-12 gap-8">
            ...
            <section class="xl:col-span-8 w-full">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                    ...
                    <div class="bg-white rounded-xl shadow-lg border border-gray-200/80 hover:shadow-xl hover:border-blue-300 transition-all duration-300 flex flex-col overflow-hidden group">
                        {{-- Gambar Artikel --}}
                        <div class="h-44 w-full flex items-center justify-center bg-gray-100 overflow-hidden">
                            ...
                            @if($artikel->thumbnail)
                                <img src="{{ asset('storage/' . $artikel->thumbnail) }}" alt="{{ $artikel->judul }}" class="w-full h-full object-cover transition-transform duration-300 group-hover:scale-105" />
                            @else
                                <img src="{{ asset('assets/img/artikel-elemen.png') }}" alt="No Image" class="w-24 h-24 object-contain opacity-40" />
                            @endif
                            </a>
                        </div>
                        {{-- Konten Teks Artikel --}}
                        <div class="flex-1 flex flex-col p-4">
                            <h3 class="font-bold text-base text-gray-800 leading-tight mb-2 line-clamp-2">
                                <a href="{{ route('kepalabagian.artikelpengetahuan.show', $artikel->id) }}" class="hover:text-blue-700">{{ $artikel->judul }}</a>
                            </h3>
                ...
                @else
                <div class="flex flex-col items-center justify-center text-center py-20 px-6 bg-white rounded-2xl shadow-lg border">
                    ...
                    <p class="text-gray-500 mt-2 max-w-sm">Saat ini belum ada data artikel pengetahuan yang tersedia. Silakan tambahkan artikel baru.</p>
                </div>
                @endif
                 {{-- Pagination --}}
                <div class="mt-8">
            ...
            <aside class="xl:col-span-4 w-full flex flex-col gap-8">
                {{-- Kartu Role --}}
                <div class="bg-gradient-to-br from-blue-600 to-blue-800 text-white rounded-2xl shadow-lg p-7 flex items-center gap-6">
                    <div class="w-16 h-16 bg-white/20 rounded-full flex items-center justify-center">
                        <i class="fa-solid fa-user-shield text-3xl"></i>
                    </div>
                    <div>
                        <p class="text-sm opacity-80">Role Anda</p>
                ...
                <div class="bg-white rounded-2xl shadow-lg p-7 space-y-3">
                    <a href="{{ route('kepalabagian.artikelpengetahuan.create') }}" class="w-full flex items-center justify-center gap-2 px-5 py-2.5 rounded-lg bg-green-600 hover:bg-green-700 text-white font-semibold shadow-sm transition text-base">
                        <i class="fa-solid fa-plus"></i>
                        <span>Tambah Artikel</span>
                    </a>
                    <a href="{{ route('kepalabagian.kategoripengetahuan.create') }}" class="w-full flex items-center justify-center gap-2 px-5 py-2.5 rounded-lg bg-blue-600 hover:bg-blue-700 text-white font-semibold shadow-sm transition text-base">
