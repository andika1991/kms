<x-app-layout>
    ...
    <div class="py-6 max-w-4xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white dark:bg-gray-800 p-6 rounded shadow">
            <form method="POST" action="{{ route('kepalabagian.manajemendokumen.update', $dokumen->id) }}" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="mb-4">
                    ...
                    <input type="file" name="path_dokumen" class="w-full text-gray-700 dark:text-gray-200">
                    @if($dokumen->path_dokumen)
                        <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">
                            File saat ini: 
                            <a href="{{ asset('storage/' . $dokumen->path_dokumen) }}" target="_blank" class="text-blue-600 underline">
                                {{ basename($dokumen->path_dokumen) }}
                            </a>
                        </p>
                    @endif
                </div>
                ...
                <div class="mb-4 hidden" id="encrypted-key-field">
                    ...
                    <input type="text" name="encrypted_key" 
                           class="w-full rounded border-gray-300 dark:bg-gray-700 dark:text-gray-200"
                           value="{{ old('encrypted_key', $dokumen->encrypted_key) }}">
                </div>
