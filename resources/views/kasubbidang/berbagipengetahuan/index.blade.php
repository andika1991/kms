<x-app-layout>
    ...
    <div class="py-6 max-w-7xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="mb-6 flex flex-col md:flex-row md:justify-between md:items-center gap-3">
            {{-- Form Pencarian --}}
            <form method="GET" action="{{ route('kasubbidang.berbagipengetahuan.index') }}" class="w-full md:max-w-md">
                <div class="flex">
...
<h4>Daftar Kategori Pengetahuan Subbidang</h4>
@foreach($kategoriPengetahuans as $kategori)
    <div class="border p-2 mb-2">
        <strong>{{ $kategori->nama_kategoripengetahuan }}</strong>
        <div>
            <a href="{{ route('kasubbidang.kategoripengetahuan.edit', $kategori->id) }}">Edit</a> |
            <form action="{{ route('kasubbidang.kategoripengetahuan.destroy', $kategori->id) }}" method="POST" style="display:inline;">
                @csrf
                @method('DELETE')
                <button onclick="return confirm('Yakin ingin menghapus kategori ini?')" class="text-red-600">Delete</button>
            </form>
        </div>
    </div>
@endforeach
