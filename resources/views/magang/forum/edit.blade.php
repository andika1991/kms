...
@section('title', 'Edit Forum Magang')

<x-app-layout>
    <div class="min-h-screen w-full bg-[#eaf5ff]">
        {{-- HEADER ala Figma/pegawai --}}
        ...
        {{-- FORM + SIDEBAR --}}
        <form id="form-edit-magang" method="POST" action="{{ route('magang.forum.update', $grupchat->id) }}"
              class="px-4 md:px-8 pt-8 pb-10 grid grid-cols-1 lg:grid-cols-3 gap-8 max-w-7xl mx-auto" autocomplete="off">
            @csrf
            @method('PUT')
            ...
            <section class="lg:col-span-2 space-y-6">
                ...
                <article class="bg-white rounded-2xl shadow-lg p-6 md:p-8">
                    <header class="mb-6">
                        <h3 class="font-bold text-lg text-gray-900">Edit Forum Diskusi</h3>
                        <p class="text-sm text-gray-500">Perbarui informasi forum di bawah ini.</p>
                    </header>

                    <label class="block mb-4">
                        <span class="block text-sm font-semibold text-gray-700 mb-1">Nama Forum</span>
                        <input name="nama_grup" value="{{ old('nama_grup', $grupchat->nama_grup) }}" required
                               class="w-full rounded-xl border border-gray-300 px-4 py-3 bg-white text-base focus:ring-2 focus:ring-blue-500">
                    ...
                    <label class="block mt-6">
                        ...
                        <input name="grup_role" value="{{ old('grup_role', $grupchat->grup_role) }}"
                               class="w-full rounded-xl border border-gray-300 px-4 py-3 bg-white text-base focus:ring-2 focus:ring-blue-500"
                ...
                <article class="bg-white rounded-2xl shadow-lg p-6 md:p-8">
                    <div class="flex items-center justify-between">
                        ...
                        <label for="is_private" class="relative inline-flex cursor-pointer select-none items-center">
                            ...
                            <span class="block w-14 h-8 rounded-full bg-gray-200 peer-checked:bg-blue-300"></span>
                            <span class="dot absolute left-1 top-1 w-6 h-6 rounded-full bg-white transition-transform"></span>
                        </label>
                    </div>

                    {{-- Umum: pilih Bidang (SEMUA bidang) --}}
                    <label id="bidang-field" class="block mt-6 @if(old('is_private', $grupchat->is_private)) hidden @endif">
                        <span class="block text-sm font-semibold text-gray-700 mb-1">Bidang (Untuk Grup Umum)</span>
                        <select name="bidang_id"
                                class="w-full rounded-xl border border-gray-300 px-4 py-3 bg-white text-base focus:ring-2 focus:ring-blue-500">
                            <option value="">-- Pilih Bidang --</option>
                            @foreach($bidangs as $b)
                                <option value="{{ $b->id }}" {{ old('bidang_id', $grupchat->bidang_id)==$b->id?'selected':'' }}>
                    ...
                    {{-- Private: pilih anggota (SEMUA user; preselected anggota) --}}
                    <label id="users-field" class="block mt-6 @if(!old('is_private', $grupchat->is_private)) hidden @endif">
                        <span class="block text-sm font-semibold text-gray-700 mb-2">Pilih Anggota Grup (Untuk Grup Private)</span>
                        <select id="user-select" name="pengguna_id[]" multiple placeholder="Cari & pilih anggota...">
                            @foreach($users as $u)
                                <option value="{{ $u->id }}" {{ in_array($u->id, $anggota_ids) ? 'selected' : '' }}>
                                    {{ $u->decrypted_name ?? $u->name }} ({{ $u->decrypted_email ?? $u->email }})
            ...
            <aside class="space-y-6">
                <div class="bg-gradient-to-br from-blue-600 to-blue-800 text-white rounded-2xl shadow-lg p-8 text-center">
                ...
                <div class="grid grid-cols-2 gap-3">
                    ...
                    <a href="{{ route('magang.forum.index') }}" id="btn-cancel"
    ...
    document.addEventListener('DOMContentLoaded', () => {
        // TomSelect (jika tersedia global)
        if (window.TomSelect) {
        ...
        // Confirm submit (Simpan)
        const form = document.getElementById('form-edit-magang');
        ...
        form.addEventListener('submit', (e) => {
            ...
            Swal.fire({
                ...
                html: `Perubahan${nama ? ` untuk <b>"${nama}"</b>` : ''} akan disimpan.`,
                icon: 'question',
        ...
        // Confirm cancel (Batalkan) — ada jarak antar tombol
        document.getElementById('btn-cancel')?.addEventListener('click', (ev) => {
            ev.preventDefault();
            const href = ev.currentTarget.getAttribute('href') || '{{ route('magang.forum.index') }}';
            Swal.fire({
    ...
    <style>
        ...
        .ts-control:focus-within{--tw-ring-color:rgb(59 130 246 / .45);border-color:#2563EB!important;box-shadow:0 0 0 2px var(--tw-ring-color)!important}
    </style>
</x-app-layout>
