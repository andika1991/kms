...
/** @var \Illuminate\Support\Collection<int, mixed> $viewers
 *  $viewers item contoh: ['pengguna' => (User), 'viewed_at' => datetime]
 */
$viewers = $viewers ?? collect();
@endphp

@section('title', 'Lihat Dokumen Sekretaris')
...
<x-app-layout>
            ...
            <section class="xl:col-span-8 w-full">
                <div class="bg-white rounded-2xl shadow-lg p-4 md:p-10 flex flex-col gap-4">
                    ...
                    <nav class="mb-1 text-xs text-gray-400">
                        <a href="{{ route('sekretaris.manajemendokumen.index') }}" class="hover:underline">Dokumen &gt; Daftar Dokumen</a>
                    </nav>

                    {{-- Judul --}}
                    <h1 class="text-xl md:text-2xl font-bold text-gray-800">{{ $dokumen->nama_dokumen }}</h1>

                    {{-- Meta row (kategori, uploader) --}}
                    <div class="flex flex-wrap gap-x-4 gap-y-1 text-sm text-gray-500">
                        <div>
                            <span>Kategori:</span>
                            <span class="font-semibold text-blue-600">{{ $dokumen->kategoriDokumen->nama_kategoridokumen ?? '-' }}</span>
                        </div>
                    ...
                    {{-- TOP BAR: tanggal kiri, views + bagikan kanan --}}
                    <div class="mt-3 flex items-start justify-between gap-4 border-b-2 border-gray-300 pb-4">
                        {{-- Tanggal unggah (pojok kiri) --}}
                        <div class="text-sm font-medium text-gray-600">
                            {{ \Carbon\Carbon::parse($dokumen->created_at)->translatedFormat('d/m/Y') }}
                        </div>

                        {{-- Views + Bagikan (pojok kanan) --}}
                        <div class="flex flex-col items-end gap-2 w-full sm:w-auto">
                            {{-- Views (ikon + avatars kecil) --}}
                            <div class="flex items-center gap-2 text-gray-500">
                                <i class="fa-regular fa-eye"></i>
                                <div class="flex -space-x-2">
                                    @foreach ($viewers->take(5) as $viewer)
                                        <img
                                            src="{{ $viewer->pengguna->avatar_url ?? 'https://ui-avatars.com/api/?name='.urlencode($viewer->pengguna->name) }}"
                                    ...
                                    @endforeach
                                    @if($viewers->count() > 5)
                                        <div class="w-6 h-6 rounded-full bg-gray-200 border-2 border-white text-[10px] flex items-center justify-center shadow">
                                            +{{ $viewers->count() - 5 }}
                                        </div>
                                    @endif
                                </div>
                                <span class="text-xs">Dilihat {{ $viewers->count() }}x</span>
                            </div>

                            {{-- Tombol Bagikan (biru, menuju halaman bagikan) --}}
                            <a href="{{ route('aksesdokumen.bagikan', $dokumen->id) }}"
                               class="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-[#2563eb] hover:bg-[#174f97] text-white font-semibold shadow transition text-sm"
                    ...
                    {{-- Konten bawah: preview + deskripsi --}}
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4">
                        ...
                        <div>
                            @if ($dokumen->path_dokumen && \Illuminate\Support\Facades\Storage::disk('public')->exists($dokumen->path_dokumen))
                                @if ($ext === 'pdf')
                                    <iframe
                                        src="{{ asset('storage/' . $dokumen->path_dokumen) }}"
                                ...
                                {{-- Download tetap di bawah preview --}}
            ...
            <aside class="xl:col-span-4 w-full flex flex-col gap-8 mt-8 xl:mt-0">
                <div
                    ...
                    <div>
                        ...
                        <p class="text-xs">Lihat dokumen kegiatan atau knowledge sharing di sini.</p>
                    </div>
                </div>
    ...
    <script>
        // masih ada kalau mau dipakai untuk salin tautan lain
