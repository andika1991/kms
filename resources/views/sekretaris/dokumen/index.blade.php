<x-app-layout>
    ...
    <div class="py-6 max-w-7xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="flex justify-between mb-6">
            <form action="{{ route('sekretaris.manajemendokumen.index') }}" method="GET" class="flex w-full max-w-md">
                <input 
                    type="text" 
                    name="search" 
            ...
            @foreach($kategori as $index => $kat)
                <tr class="{{ $index % 2 == 0 ? 'bg-white dark:bg-gray-800' : 'bg-gray-50 dark:bg-gray-700' }}">
                    ...
                    <td class="px-4 py-2 border-b border-gray-300 dark:border-gray-600 flex gap-2">
                        ...
                        <!-- Tombol Hapus -->
                        <form action="{{ route('sekretaris.kategori-dokumen.destroy', $kat->id) }}" method="POST" onsubmit="return confirm('Hapus kategori ini?');">
                            @csrf
                            @method('DELETE')
                            <button type="submit" 
