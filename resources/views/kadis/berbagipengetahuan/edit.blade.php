...
@section('title', 'Edit Pengetahuan Kadis')
...
<x-app-layout>
    <div class="w-full min-h-screen bg-[#eaf5ff] pb-32">
        ...
        <form id="edit-artikel-form" method="POST"
            action="{{ route('kadis.berbagipengetahuan.update', $artikelpengetahuan->id) }}"
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                window.location.href =
                    "{{ route('kadis.berbagipengetahuan.show', $artikelpengetahuan->id) }}";
            }
        });
    });
    </script>
